...
public class BlogPostDAO {
    ...
    public List<Document> findByDateDescending(int limit) {
        ...
        FindIterable<Document> list = postsCollection.find().sort(new Document("date",-1)).limit(limit);
        for(Document post : list)
        {
            posts.add(post);
        }
        return posts;
    ...
    public String addPost(String title, String body, List tags, String username) {
        ...
        post.append("title",title).append("body",body).append("permalink",permalink).append("author",username)
                .append("tags",tags).append("comments",new ArrayList<Document>()).append("date",new Date());
